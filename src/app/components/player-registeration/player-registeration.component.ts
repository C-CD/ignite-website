...
export class PlayerRegisterationComponent implements OnInit {
  ...
  test(){
    const directData = {
      ...
      desc: "Thanks your registration form has been submitted successfully, you will be contacted with instructions on the next steps.",
      success: true
    };
  ...
  validateFormData() {
    this.formDataGroup = this.formBuilder.group({
      ...
      primary_name: new FormControl('', Validators.compose([Validators.required])),
      primary_cert_no: new FormControl('', Validators.compose([Validators.required])),
      primary_cert_file: new FormControl('', Validators.compose([Validators.required])),
      primary_address: new FormControl('', Validators.compose([Validators.required])),
      ...
      guardian_state: new FormControl('', Validators.compose([Validators.required])),
      guardian_lga: new FormControl('', Validators.compose([Validators.required])),
      guardian_city: new FormControl('', Validators.compose([Validators.required])),
      ...
      year: new FormControl(moment().format('YYYY'), Validators.compose([Validators.required])),
    });
  }
    ...
    this.loadingService.quickLoader().then(() => {
      this.uploadFiles(formData).then((formData) => {
        ...
        this.playerRegService.addPlayersRegistration({
          ...
          primary_cert_no: formData.primary_cert_no,
          primary_cert_file: formData.primary_cert_file,
          ...
          guardian_state: formData.guardian_state,
          guardian_lga: formData.guardian_lga,
          ...
          const directData = {
            ...
            desc: "Error registering your information kindly try again.",
            success: false
          };
          this.openModal(directData);
        }).finally(() => {
  ...
  async uploadFiles(formData: any) {
    try {
      ...
      if (formData.primary_cert_file.length && this.uploads && this.uploads.cert) {
        const primary_cert_file: any = await this.uploadService.startFileUpload(this.uploads.cert, 'playerPrimaryCertificateImage', 0, 'file');
        if (primary_cert_file) formData.primary_cert_file = primary_cert_file.filepath;
      }
    }
    catch (error) {
      console.log(error);
